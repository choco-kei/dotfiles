...
const vec4 TRAIL_COLOR = vec4(0.78, 0.80, 0.85, 1.0);
const vec4 CURRENT_CURSOR_COLOR = TRAIL_COLOR;
const vec4 PREVIOUS_CURSOR_COLOR = TRAIL_COLOR;
const vec4 TRAIL_COLOR_ACCENT = vec4(0.65, 0.70, 0.80, 1.0);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    // --- 修正箇所：ここから ---
    // トレイルの太さを調整（0.05でカーソルの5%の太さになります）
    float trailWidthFactor = 0.05;

    // 中心に寄せるためのオフセット計算
    vec2 trailSize = currentCursor.zw * trailWidthFactor;
    vec2 margin = currentCursor.zw * (1.0 - trailWidthFactor) * 0.5;

    // 頂点を中心に合わせて再計算
    // v0, v3 は下側、v1, v2 は上側の頂点
    vec2 v0 = vec2(currentCursor.x + margin.x + trailSize.x * vertexFactor,          currentCursor.y - currentCursor.w + margin.y);
    vec2 v1 = vec2(currentCursor.x + margin.x + trailSize.x * invertedVertexFactor, currentCursor.y - margin.y);
    vec2 v2 = vec2(previousCursor.x + margin.x + trailSize.x * invertedVertexFactor, previousCursor.y - margin.y);
    vec2 v3 = vec2(previousCursor.x + margin.x + trailSize.x * vertexFactor,          previousCursor.y - previousCursor.w + margin.y);
    // --- 修正箇所：ここまで ---
